...
A future Home Assistant policy layer should allow external temperature control while preserving better user experience. For example, the fireplace could start at a higher flame for warmup, reduce flame as the room approaches target, and then either hold minimum flame, turn the flame off, or power off depending on user preference.
...
Lower-cost support is planned for CC1101-based devices, especially ESP32 + CC1101 designs. This is likely the best long-term deployment model for many users because it avoids direct USB attachment to the Home Assistant host and allows a small dedicated RF node to live near the fireplace.
